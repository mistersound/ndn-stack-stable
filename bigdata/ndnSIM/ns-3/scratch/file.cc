...
main(int argc, char* argv[])
{
  ...
  ndn::AppHelper adminHelper("ns3::ndn::Admin");
	...
	adminHelper.Install(nodes.Get(0));//Names::Find<Node>("Node8"));                      // first node
		...
		storageHelperA.Install(nodes.Get(1));
		storageHelperA.Install(nodes.Get(3));
		storageHelperA.Install(nodes.Get(4));
		storageHelperA.Install(nodes.Get(5));
...
Simulator::Schedule(Seconds(18), ndn::LinkControlHelper::FailLink, nodes.Get(1), nodes.Get(2));
  ...
  Simulator::Run();
     ...
     for(auto it = myContentStore.begin();
         ...
         ++it)
     {
        cout<<it->getName()<<endl<<endl;
     }

     //myContentStore.dump();
    //cout<<myContentStore.size()<<endl<<endl;
    //myContentStore.erase("/lacl/data/stop/0/1/");
